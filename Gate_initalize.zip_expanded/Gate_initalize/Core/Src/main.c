...
void handle_PA4_interrupt(void)
{
    if(flag1 == 0)
    	flag1 =1;
}

void handle_PA5_interrupt(void)
{
    if(flag2 ==0)
    	flag2 =1;
}
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
    if (GPIO_Pin == GPIO_PIN_4)
        handle_PA4_interrupt();
    else if (GPIO_Pin == GPIO_PIN_5)
        handle_PA5_interrupt();
}
...
int main(void)
{
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  lcd_put_cur(0,0);
	  lcd_send_string("____CAR SLOT____");
